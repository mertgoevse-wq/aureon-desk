...
export const CONNECTOR_PRESETS: ConnectorPreset[] = [
  {
    ...
    limitations: ['Remote provider privacy and billing policies apply.', 'Aureon does not bundle or expose provider keys in source code.'],
    mockMode: false,
  ...
  },
  {
    ...
    riskLevel: 'medium',
    status: 'planned',
    setupInstructions: ['Install the future Aureon phone companion app.', 'Pair on the same local network.', 'Approve permissions on the phone.'],
  ...
  },
  {
    ...
    riskLevel: 'high',
    status: 'planned',
    setupInstructions: ['Use the official WhatsApp Business Platform only.', 'Verify business/account permissions outside Aureon.', 'Require approval before any message send.'],
    testConnectionBehavior: 'mock-only',
